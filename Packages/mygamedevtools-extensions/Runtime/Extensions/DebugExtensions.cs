...
namespace MyGameDevTools.Extensions
{
    ...
    {
        const int _identSpaces = 2;

        public static void LogCollection<T>(IEnumerable<T> collection) => Debug.Log(CollectionToString(collection));
        ...
        static string CollectionToString<T>(IEnumerable<T> collection, int identLevel = 0)
        {
            ...
            sb.AppendFormat("{0}{1} collection with {2} elements{3}\n", identLevel > 0 ? new string(' ', _identSpaces * identLevel) : string.Empty, typeof(T).Name, count, count > 0 ? ":" : string.Empty);
            for (int i = 0; i < count; i++)
                sb.AppendFormat("{0}[{1}] {2}\n", new string(' ', _identSpaces * (identLevel + 1)), i, collection.ElementAt(i).ToString());
            return sb.ToString();
        }
    }
}